...
def extrair_mes_ano(linhas):
    ...
    for linha in linhas:
        # Procura por padrões como "Mensalista Fevereiro de 2019" ou "Folha Mensal"
...
def processar_pagina(page):
    ...
    # Processa apenas até encontrar "Total de Vencimentos" (para evitar duplicatas na mesma página)
    processando = True
    ...
    for i, linha in enumerate(linhas):
        ...
        # Para de processar quando encontrar "Total de Vencimentos" (primeira aparição)
        if "Total de Vencimentos" in linha and linha_processada:
            print("Parando processamento após primeira aparição de 'Total de Vencimentos'")
            processando = False
            break
        
        # Marca que já processou pelo menos uma linha
        if "HORAS NORMAIS" in linha or "INSALUBRIDADE" in linha or any(palavra in linha for palavra in ["INSS", "IRRF", "VALE"]):
